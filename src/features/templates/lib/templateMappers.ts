...
export const mapReadToManagedTemplate = (template: ApiTemplateRead): ManagedTemplate => {
  ...
  const sections: Section[] = template.sections
    .sort((a, b) => a.order_index - b.order_index)
    .map((section) => {
      const mappedStyles: SectionStyle[] = (section.allowed_styles ?? []).map(mapStyleToFrontend);
      const resolvedStyles: SectionStyle[] = mappedStyles.length > 0 ? mappedStyles : ["paragraph"];
